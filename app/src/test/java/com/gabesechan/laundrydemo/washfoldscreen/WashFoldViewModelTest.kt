...
class WashFoldViewModelTest {
    ...
    private fun awaitDataLoaded(viewModel: WashFoldViewModel) = runBlocking {
        viewModel.dataLoaded.first { it }
    }

    @Test
    fun testInitSuccessLoadsWashFoldItemAndEnablesPickupDates() {
        val laundromatInfoServer = mockk<LaundromatInfoServer> {
            ...
            coEvery { items() } returns NetworkResponse(
                true, null, emptyList(),
                ItemsResponse(listOf(washFoldItem, dryCleanItem))
            )
        }
    ...
    @Test
    fun testInitNetworkErrorSetsNetworkError() {
        val laundromatInfoServer = mockk<LaundromatInfoServer> {
    ...
    @Test
    fun testBookNetworkErrorSetsNetworkError() = runTest {
        val laundromatInfoServer = mockk<LaundromatInfoServer> {
            coEvery { availableTimes() } returns NetworkResponse(true, null, emptyList(), availableTimesResponse)
            coEvery { items() } returns NetworkResponse(true, null, emptyList(), ItemsResponse(listOf(washFoldItem)))
        }
    ...
    @Test
    fun testBookEnabledRequiresDropoffTimeAndAddress() = runTest {
    ...
    @Test
    fun testBookEnabledFalseWhenDropoffTimeNotSet() = runTest {
        Dispatchers.setMain(StandardTestDispatcher(testScheduler))
        ...
        assertNull(viewModel.dropoffDateValues.value.curSelectedTime)
        assertFalse(viewModel.bookEnabled.value)
    ...
    fun testBookEnabledFalseWhenSelectedAddressIsNull() = runTest {
        ...
        val viewModel = WashFoldViewModel(laundromatInfoServer, userRepository(noAddressUser), orderServer)
        awaitDataLoaded(viewModel)
    ...
    @Test
    fun testBookEnabledFalseWhilePosting() = runTest {
        Dispatchers.setMain(StandardTestDispatcher(testScheduler))
